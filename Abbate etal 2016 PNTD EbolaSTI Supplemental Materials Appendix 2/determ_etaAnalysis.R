###################################################################
# 
#  Deterministic model simulations - Eta parameter analysis
#
#  Accompanying:
#
#  JL Abbate, C-L Murall, H Richner, C Althaus (2015) Potential 
#  impact of sexual transmission of Ebola virus on the epidemic 
#  in West Africa                                  
#                                                                 
#  December 11, 2015                 
###################################################################
...
alpha =  1/((365/12)*3-3.9) # daily rate of full recovery (from C --> R)
...
model.wot <- function(t, x, parms) {
  with(as.list(c(parms,x)),{ 
    ...
    beta <- ifelse(t<tau,beta0,(beta1 + (beta0-beta1)*exp(-k*(t-tau))))  #old: (beta0-beta1)*exp(-k*(t-tau))
...
etaVals <- c(0.0005, 0.001, 0.002)   # range of eta values 

simulation <- list()                  # output of loop

#loop on the number of runs
for(i in 1:length(etaVals))
{
  pars <-  c(R0 = R0, R1 = R1,beta0 = beta0, beta1 = beta1, sigma = sigma,     
             gamma = gamma, f = f,  p = p, q = q, alpha= alpha, k= k,tau= tau,
             eta = etaVals[i])
  
  #run model.wt
  simulation <- as.data.frame(ode(init,timepoints2,model.wt,parms=pars))
...
# Plots
# -----
# pdf("deterministic eta sensitivity.pdf")
...
legend("topright",legend=c("Exposed","Infected","Recovered","Deaths","Convalescent","Cum. no. cases","Day of last case"), lty=1, lwd=wd,col=c("orange","red","darkgreen","brown","blue","purple","black"),bty="n",cex=0.65)
...
legend("topright",legend=c("0.0005", "0.001", "0.002"),lwd=wd, lty=c(int2,int3,int4), title="eta", cex=0.65)

plot(timepoints, simST3[c(1:endt)], lty=int4,lwd=wd, ylim=c(1e-2,1e2),type="l",xlab="Time (days)", ylab="Cumulative number of sexual transmissions",frame=FALSE)
lines(timepoints, simST2[c(1:endt)], lty=int3,lwd=wd)
lines(timepoints, simST1[c(1:endt)], lty=int2,lwd=wd)

mtext("Sensitivity to sexual transmission probability", outer = TRUE, cex = 1) #title outside the margins
